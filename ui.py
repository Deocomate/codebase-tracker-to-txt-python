...
class CodebaseTrackerUI:
    ...
    def _copy_structure_path(self):
        structure_var = getattr(self, 'structure_path_var', None)
        if structure_var:
            path = structure_var.get()
            if path:
                self.root.clipboard_clear()
                self.root.clipboard_append(path)
                self.status_var.set("Structure path copied to clipboard!")
    ...
    def _open_structure_file(self):
        structure_file = self.output_stats.get('structure_file')
        if structure_file:
